...
describe('logic - general report', () => {
    ...
    it('should succeed listing all agents', async () => {
        ...
        const report = await retrieveReport(idAdmin)
        console.log(report)
        
            expect(report.calls).that.is.a('number')
            expect(report.answered).that.is.a('number')
            expect(report.pending).that.is.a('number')
            expect(report.visits).that.is.a('number')
            expect(report.fails).that.is.a('number')
            expect(report.calling).that.is.a('number')
            expect(report.calls).to.equal(report.answered + report.pending)
    })
